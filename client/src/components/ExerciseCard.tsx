...
interface ExerciseCardProps {
  exercise: Exercise;
  workoutType: "strength" | "cardio" | "core" | "sports"; // Add workoutType prop
...
export default function ExerciseCard({ exercise, workoutType, onUpdate, onDelete, startOpen = false }: ExerciseCardProps) {
  ...
  // When an exercise name is selected, find its category and update the state
  const handleExerciseSelect = (exerciseName: string) => {
    const selected = masterExerciseList.find(ex => ex.name === exerciseName);
    if (selected) {
      setSelectedExerciseCategory(selected.category);
      onUpdate({ name: exerciseName, type: selected.category as any });
      // Automatically add a set when an exercise is selected for the first time
      if (exercise.sets.length === 0) {
        addSet(selected.category);
      }
    } else {
      onUpdate({ name: "" });
    }
  ...
  const displayCategory = selectedExerciseCategory || exercise.type;
    ...
    <div className={cn(
      ...
      {isOpen && (
        <div className="p-4 border-t border-dark-border space-y-4">
          ...
          {/* Render inputs based on the selected exercise's category */}
          {exercise.name && (
            <>
              {/* Sets Header */}
